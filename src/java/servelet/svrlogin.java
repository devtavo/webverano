...
//            out.println("</html>");
            ...
            perfilbr perfil = new perfilbr();
            int res = usuario.validarAcceso(usu);
            int per1=perfil.loginAdministrativo(usu);
            int per2=perfil.logiFuncionario(usu);
            
                if (res==1){
                    if(per1>0 ){
                        request.setAttribute("datos", "");
            
                    RequestDispatcher rd = getServletContext().getRequestDispatcher("/index_administrativo.html");
                      rd.forward(request, response);
                      
                    }else if(per2<0 ){
                        request.setAttribute("datos", "");
            
                    RequestDispatcher rd = getServletContext().getRequestDispatcher("/index_funcionario.html");
                      rd.forward(request, response);
                    }
